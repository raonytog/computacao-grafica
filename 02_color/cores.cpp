...
void motion(int x, int y){
    ...
    if (choosingColor){
        ...
        /** calculo para atualizar o ponto de projecao */
        float denominador = (pRx - pGx)*(pBy - pCliqueY) - (pRy - pGy)*(pBx - pCliqueX);
        if (denominador == 0) { printf("Divisao por zero\n"); return; }

        float t = ( (pRx - pBx)*(pBy - pCliqueY) - (pRy - pBy)*(pBx - pCliqueX) )/denominador;
        pProjX = pRx + t*(pGx - pRx);
        pProjY = pRy + t*(pGy - pRy);
 
        /** calculo para atualizacao das cores de fundo */
        /**
            traducao:
            K = R       I = G       J = B
            
            orginal:                                    transformado:
            Fr = (1.0 - |PK|/|IK|) * |CJ|/|PJ|;         Fr = (1 - PR/GR) * CB/PB
            Fg = (1.0 - |PI|/|IK|) * |CJ|/|PJ|;         Fg = (1 - PG/GR) * CB/PB
            Fb = (1.0 - |CJ|/|PJ|);                     Fb = 1 - CB/PB
         */
        gR = (1.0 - distance(pProjX, pProjY, pRx, pRy)/distance(pGx, pGy, pRx, pRy)) * distance(pCliqueX, pCliqueY, pBx, pBy)/distance(pProjX, pProjY, pBx, pBy);
        gG = (1.0 - distance(pProjX, pProjY, pGx, pGy)/distance(pGx, pGy, pRx, pRy)) * distance(pCliqueX, pCliqueY, pBx, pBy)/distance(pProjX, pProjY, pBx, pBy);
        gB = (1.0 - distance(pCliqueX, pCliqueY, pBx, pBy)/distance(pProjX, pProjY, pBx, pBy));
...
void mouse(int button, int state, int x, int y) {
    ...
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) { choosingColor = 1; } 
    ...
    else if (button == GLUT_LEFT_BUTTON && state == GLUT_UP) { choosingColor = 0; }
    else if (button == GLUT_RIGHT_BUTTON && state == GLUT_UP) { draggingPointR = draggingPointG = draggingPointB = 0; }
