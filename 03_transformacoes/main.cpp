...
void idle(void) {
    ...
    //Treat keyPress
    if(keyStatus[(int)('a')]) { robo.MoveEmX(-inc); }
    if(keyStatus[(int)('d')]) { robo.MoveEmX(inc); }
    ...
    if(tiro) {
        ...
        //Trata colisao
        if (alvo.Atingido(tiro)) { alvo.Recria(rand()%500 - 250, 200); }
        if (!tiro->Valido()) { delete tiro; tiro = NULL; }
    }
    ...
    if (animate){
        static int dir = 1;
        if (robo.ObtemX() > (ViewingWidth/2)) { dir *= -1; }
        else if (robo.ObtemX() < -(ViewingWidth/2)) { dir *= -1; }
