...
//    glShadeModel (GL_SMOOTH);
    ...
    glOrtho (-3, 3, 
             -3*(GLfloat)windowSize/(GLfloat)windowSize, 3*(GLfloat)windowSize/(GLfloat)windowSize, 
             1.0, 15.0);   
}
...
void DrawObj(double size) {
    ...
    // glutSolidCone(0.5, size*2, 20, 10);
}
...
void display(void) {
    ...
    glLoadIdentity();
    // gluLookAt(0,2,5,  // posiscao da camera
    //           0,0,0,  // mirando para a origem
    //           0,1,0); // upvector apontando para y

    gluLookAt(0,2,5,  // posiscao da camera
              0,0,0,  // mirando para a origem
              0,1,0); // upvector apontando para y
   
    GLfloat light_position[] = { 10, 0.0, 0, 1.0 };
